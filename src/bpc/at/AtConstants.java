...
public class AtConstants {
    ...
    private AtConstants() {
        ...
        MAX_PAYLOAD_FOR_BLOCK.put((short) 1, (BPC.getFluxCapacitor().getValue(FluxValues.MAX_PAYLOAD_LENGTH)) / 2L); //use at max half size of the block.
        ...
        MAX_PAYLOAD_FOR_BLOCK.put((short) 2, (BPC.getFluxCapacitor().getValue(FluxValues.MAX_PAYLOAD_LENGTH)) / 2L); //use at max half size of the block.
    ...
    public short atVersion(int blockHeight) {
        return BPC.getFluxCapacitor().getValue(FluxValues.AT_VERSION, blockHeight);
    }
