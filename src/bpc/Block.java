...
public class Block {
  ...
  Block(int version, int timestamp, long previousBlockId, long totalAmountNQT, long totalFeeNQT,
    ...
    if (payloadLength > BPC.getFluxCapacitor().getValue(FluxValues.MAX_PAYLOAD_LENGTH, height) || payloadLength < 0) {
    ...
    if (transactions != null) {
      ...
      if (blockTransactions.get().size() > (BPC.getFluxCapacitor().getValue(FluxValues.MAX_NUMBER_TRANSACTIONS, height))) {
        throw new BurstException.NotValidException(
  ...
  private TransactionDb transactionDb() {
    return BPC.getDbs().getTransactionDb();
  }
