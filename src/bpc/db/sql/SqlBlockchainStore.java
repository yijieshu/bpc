...
public class SqlBlockchainStore implements BlockchainStore {
  ...
  private final TransactionDb transactionDb = BPC.getDbs().getTransactionDb();
  private final BlockDb blockDb = BPC.getDbs().getBlockDb();
  ...
  public Collection<Block> getBlocks(int from, int to) {
    return Db.useDSLContext(ctx -> {
      int blockchainHeight = BPC.getBlockchain().getHeight();
      return
  ...
  public Collection<Transaction> getTransactions(Account account, int numberOfConfirmations, byte type, byte subtype, int blockTimestamp, int from, int to, boolean includeIndirectIncoming) {
    int height = numberOfConfirmations > 0 ? BPC.getBlockchain().getHeight() - numberOfConfirmations : Integer.MAX_VALUE;
    if (height < 0) {
      throw new IllegalArgumentException("Number of confirmations required " + numberOfConfirmations + " exceeds current blockchain height " + BPC.getBlockchain().getHeight());
    }
    return Db.useDSLContext(ctx -> {
