...
public class SqlTransactionDb implements TransactionDb {
  ...
  @Override
  public List<Transaction> getTransactionsByHeightAndRecipient(Integer height) {
    return Db.useDSLContext(ctx -> {
      return ctx.selectFrom(TRANSACTION)
              ...
              .fetch(record -> {
                try {
                  ...
                  throw new RuntimeException("Transaction already in database for height = " + Convert.toUnsignedLong(height) + " does not pass validation!", e);
                }
              });
    });
  }
