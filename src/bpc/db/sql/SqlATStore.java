...
public class SqlATStore implements ATStore {
  ...
  private void saveATState(DSLContext ctx, brs.at.AT.ATState atState) {
    ctx.mergeInto(AT_STATE, AT_STATE.AT_ID, AT_STATE.STATE, AT_STATE.PREV_HEIGHT, AT_STATE.NEXT_HEIGHT, AT_STATE.SLEEP_BETWEEN, AT_STATE.PREV_BALANCE, AT_STATE.FREEZE_WHEN_SAME_BALANCE, AT_STATE.MIN_ACTIVATE_AMOUNT, AT_STATE.HEIGHT, AT_STATE.LATEST)
            ...
            .values(atState.getATId(), brs.at.AT.compressState(atState.getState()), atState.getPrevHeight(), atState.getNextHeight(), atState.getSleepBetween(), atState.getPrevBalance(), atState.getFreezeWhenSameBalance(), atState.getMinActivationAmount(), BPC.getBlockchain().getHeight(), true)
            .execute();
  }
  ...
  private void saveAT(DSLContext ctx, brs.at.AT at) {
    ctx.insertInto(
      ...
      brs.at.AT.compressState(at.getApCodeBytes()), BPC.getBlockchain().getHeight()
    ).execute();
  }
  ...
  public List<Long> getOrderedATs() {
    return Db.useDSLContext(ctx -> {
      return ctx.selectFrom(
              ...
              AT_STATE.NEXT_HEIGHT.lessOrEqual(BPC.getBlockchain().getHeight() + 1)
      ).and(
              ACCOUNT.BALANCE.greaterOrEqual(
                      AtConstants.getInstance().stepFee(BPC.getBlockchain().getHeight())
                              * AtConstants.getInstance().apiStepMultiplier(BPC.getBlockchain().getHeight())
              )
      ).and(
