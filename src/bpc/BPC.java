...
public final class BPC {
  ...
  public static final String APPLICATION = "BPC";

  private static final String DEFAULT_PROPERTIES_NAME = "brs-default.properties";

  private static final Logger logger = LoggerFactory.getLogger(BPC.class);
  ...
  private static PropertyService loadProperties() {
    ...
    logger.info("Initializing Bitcoin-PC (BPC) version {}", VERSION);
    try (InputStream is = ClassLoader.getSystemResourceAsStream(DEFAULT_PROPERTIES_NAME)) {
      if (is != null) {
        ...
        if (configFile != null) {
          ...
          defaultProperties.load(new FileInputStream("D:/BPC/bpc/conf/" + DEFAULT_PROPERTIES_NAME));
//          throw new RuntimeException(DEFAULT_PROPERTIES_NAME + " not in classpath and system property " + DEFAULT_PROPERTIES_NAME + " not defined either");
  ...
  public static void main(String[] args) {
    Runtime.getRuntime().addShutdownHook(new Thread(BPC::shutdown));
    init();
  }
  ...
  private static void loadWallet(PropertyService propertyService) {
    ...
    BPC.propertyService = propertyService;
    ...
    try {
      ...
      logger.info("BPC {} started successfully.", VERSION);
    ...
    (new Thread(BPC::commandHandler)).start();
  }
  ...
  public static void shutdown(boolean ignoreDBShutdown) {
    ...
    logger.info("BPC {} stopped.", VERSION);
