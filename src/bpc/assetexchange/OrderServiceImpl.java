...
class OrderServiceImpl {
  ...
  private Ask getNextAskOrder(long assetId) {
    return BPC.getStores().getOrderStore().getNextOrder(assetId);
  }

  private Bid getNextBidOrder(long assetId) {
    return BPC.getStores().getOrderStore().getNextBid(assetId);
  }
  ...
  private void matchOrders(long assetId) {
    ...
    while ((askOrder = getNextAskOrder(assetId)) != null
      ...
      Trade trade = tradeService.addTrade(assetId, BPC.getBlockchain().getLastBlock(), askOrder, bidOrder);
