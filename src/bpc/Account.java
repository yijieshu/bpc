...
public class Account {
  ...
  private static BurstKey.LongKeyFactory<Account> accountBurstKeyFactory() {
    return BPC.getStores().getAccountStore().getAccountKeyFactory();
  }

  private static VersionedBatchEntityTable<Account> accountTable() {
    return BPC.getStores().getAccountStore().getAccountTable();
  }
  ...
  public Account(long id) {
    ...
    this.creationHeight = BPC.getBlockchain().getHeight();
  }
  ...
  public boolean setOrVerify(byte[] key, int height) {
    return BPC.getStores().getAccountStore().setOrVerify(this, key, height);
  }
