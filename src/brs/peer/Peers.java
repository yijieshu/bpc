...
public final class Peers {
  ...
  public static boolean isSupportedUserAgent(String header) {
    if(header == null || header.isEmpty() || ! header.trim().startsWith("BPC/")) {
      return false;
    } else {
      try {
        return isHigherOrEqualVersion(MIN_VERSION, Version.parse(header.trim().substring("BPC/".length())));
  ...
  static final int DEFAULT_PEER_PORT = 8223;
  static final int TESTNET_PEER_PORT = 7223;
